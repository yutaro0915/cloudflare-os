...
import { Archive, CheckCircle, Eye, FileCode, LockKey, Package, Plus, ShieldCheck, Trash, Warning } from '@phosphor-icons/react'
import type {
  AdminApi,
    ...
    <article className="rounded-2xl border border-kumo-line bg-kumo-base p-5 shadow-sm">
      ...
      {entry.offers.length > 0 && (
        <div className="mt-5 border-t border-kumo-line pt-4">
          ...
          {pendingOffer && (
            <div data-testid="import-review" className="mt-3 rounded-xl border border-kumo-brand bg-kumo-tint px-4 py-4">
              <div className="flex items-start gap-3">
  ...
  return (
    <article className="rounded-xl border border-kumo-line bg-kumo-base px-4 py-3">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <WorkshopButton
          tone="danger"
          onClick={state.lifecycle === 'purging' ? purge : () => setConfirmingPurge(true)}
          disabled={controller.mutating}
        >
